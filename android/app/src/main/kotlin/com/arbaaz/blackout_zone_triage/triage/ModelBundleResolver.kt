...
object ModelBundleResolver {
    ...
    private fun extractFirstRegularFile(tarFile: File, outputDir: File): File {
        FileInputStream(tarFile).use { input ->
            while (true) {
                ...
                if (headerRead < 512) {
                    throw IllegalStateException("Unexpected end of tar header in ${tarFile.name}")
                }
                if (header.all { it == 0.toByte() }) {
                    break
                }
        ...
        throw IllegalStateException("No model file found inside tar bundle: ${tarFile.name}")
    }

    private fun skipTarContent(input: FileInputStream, size: Long) {
        var remaining = size
    ...
    private fun skipTarPadding(input: FileInputStream, size: Long) {
